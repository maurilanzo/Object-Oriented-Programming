...
public class Generation {
    ...
    @MapKeyJoinColumn(name="cell_id")
    @Column(name="energy")
    private Map<Cell,CellMood> cellsMood= new HashMap<>();
    ...
    public Map<Cell, Boolean> snapCells() {
        ...
        for (Tile tile : board.getTiles()) {
            ...
            cellsMood.put(cell,cell.getMood());
    ...
    public static Generation createInitial(Game game, Board board, Map<Coord, CellType> cellTypesMap) {
        Objects.requireNonNull(game, "Game cannot be null");
        Objects.requireNonNull(board, "Board cannot be null");
        Objects.requireNonNull(cellTypesMap, "aliveCells cannot be null");

        game.clearGenerations();
        Generation init = new Generation(game, board, 0);
        
        // mark each cell with its type inside the board
        for (Map.Entry<Coord, CellType> entry : cellTypesMap.entrySet()) {
            Cell cell = board.getCell(entry.getKey()); // get the cell inside the board
            cell.setType(entry.getValue()); // set the cell to the corresponding type
            
        }

        // mark each cell with its aliveness state
        List<Coord> coordsList = cellTypesMap.keySet()
                                             .stream()
                                             .collect(Collectors.toList());
        init.setState(coordsList, true);

        // capture the current state of every cell on the board
        init.snapCells();
        game.addGeneration(init, 0);
        return init;
    }
    ...
    public Map<Cell, Integer> getEnergyStates() {
        return energyStates; // Return the map containing cell energy states
    }
